...
/// Make a chat completion request with automatic API routing.
pub async fn chat_completion(
    ...
    let (key_openai, key_anthropic, key_google, server_url) = load_local_keys();
    let (url, headers) = llm_endpoint(model, &key_openai, &key_anthropic, &key_google, &server_url);
    let body = build_llm_body(model, messages, temperature, max_tokens, stream);
    ...
    match req.send().await {
        Ok(resp) if resp.status().is_success() => {
            if stream && !model.starts_with("claude") {
                // Return raw response for streaming
                Ok(format!("__STREAM__{url}"))
            } else {
                match resp.json::<serde_json::Value>().await {
                    Ok(data) => parse_llm_response(model, &data).ok_or_else(|| "Empty response".into()),
...
/// Make a non-streaming chat completion request (simple helper).
pub async fn chat_simple(
    ...
    chat_completion(model, &messages, temperature, 4096, false).await
}
